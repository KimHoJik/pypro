...
print('예측값 : ', model.predict(x_test[:5]))  # [0. 1. 1. 0. 0.]
print('실제값 : ', y_test[:5])  # [0. 1. 1. 0. 1.]
print((model.predict(x_test) != y_test).sum())  # 254개 중 54개가 틀림
print('test로 분류 정확도 : ', model.score(x_test, y_test))  # 0.7874
print('train로 분류 정확도 : ', model.score(x_train, y_train))  # 0.7743

from sklearn.metrics import accuracy_score
pred = model.predict(x_test)
print('분류 정확도 : ', accuracy_score(y_test, pred))  # 0.7874
