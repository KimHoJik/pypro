...
# 정확도 100% 찝찝한 결과 : 과적합 문제 발생
# 모의고사 문제와본고사 문제가 똑같음
# 모델의 포용성을 위해 과적합 문제를 해결하는 방안

# 참고 : 과적합방지로 데이터수를 늘릴 수 있다.

print('---과적합 방지 목적 처리 1---')
# train / test split (보통 7 : 3, 8 : 2)
...
# 전체 행 수가 150, 학습데이터 : 4/5(120개), 검증데이터 : 1/5로 분할해 가며 학습을 진행
...
for train_index, test_index in kfold.split(features):
    ...
    cv_acc.append(acc)
print('평균 검증 정확도 : ', np.mean(cv_acc))  # 0.9199
    
print('\n---과적합 방지 목적 처리 2-1 ---')
# 대출 사기 데이터인 경우 정상과 사기 데이터가 비율적으로 편향된 경우가 많다.
# 위와 같은 불균형 데이터인 경우는 KFold 보다는 StratifiedKFold 사용
...
# 여러 개의 속성 값중max_depth, in_samples_split에 대해서 최적의 값 찾기
