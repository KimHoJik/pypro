...
# 원래 데이터 분산을최대한 보존하는 새로운 축을 찾고, 그 축에 데이터를 사영(projection) 시키는 방법(직교) 사용
...
# 시각화
plt.plot(x.T, 'o:')
plt.xticks(range(2), ['곷받침 길이', '꽃받침 폭'])
plt.xlabel('특성의 종류')
plt.ylabel('특성의 값')
plt.title('붓꽃 크기 특성')
...
"""
# x축에는 꽃받침 길이, y축에는 꽃받침 폭으로산점도 출력
df = pd.DataFrame(x)
...
# 두 개의 열의 값 패턴이 매우 유사함을 알 수 있다. 그러므로 차원 축소 가능 ==> PCA
