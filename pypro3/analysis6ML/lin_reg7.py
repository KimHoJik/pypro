...
def regScore_func(y_true, y_pred):
    ...
    print('explaind_variance_score(설명분산점수):{}'.format(explained_variance_score(y_true, y_pred)))
    print('mean_squred_error(평균제곱근오차):{}'.format(mean_squared_error(y_true, y_pred)))
    # mean_squared_error : 잔차의 제곱의 합을 표본수로 나눈 값. 분산분석의 SSE와 같음
    
regScore_func(y, y_pred)
# r2_score(결정계수):0.9987875127274646
# explaind_variance_score(설명분산점수):0.9987875127274646  # r2_score와 이 값이 다르면 에러에 편향이 있음. 모델 학습이 잘못 되었다는 뜻.
# mean_squred_error(평균제곱근오차):86.14795101998745
...
print(x[:5])
print(y[:5])
print('상관계수 : ', np.corrcoef(x, y))  # 0.004011
