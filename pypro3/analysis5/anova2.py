# 분산분석
# 강남구에 있는 GS 편의점 3개지역 알바생의 급여에 대한 평균차이를 검정하기
# 귀무 : GS 편의점 3개지역 알바생의 급여에 대한 평균의 차이가 없다
# 대립 : GS 편의점 3개지역 알바생의 급여에 대한 평균의 차이가 있다
import numpy as np
import pandas as pd
...
print(gr1, ' ', np.mean(gr2))  # 256.444
print(gr1, ' ', np.mean(gr3))  # 278.0
...
print(anova_lm(lmodel, typee = 1))
# PR(>F) 0.043589 < 0.05 이므로 귀무기각. GS 편의점 3개 지역 알바생의 급여에 대한 평균에 차이가 있다.
