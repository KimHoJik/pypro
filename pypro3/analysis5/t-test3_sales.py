# data.go.kr 사이트에서 어느 음식점매출데이터와 날씨 데이터를 이용하여 강수 여부에  따른 매출의 평균에 차이를 검정
...
print('---독립표본 t 검정 : 강수 여부에 따른 매출액의 평균에 차이가 있는가?---')
# print(data['sumRn'] > 0)
#
...
tg1 = sp[sp[:, 1] == 0, 0]  # 비안올 때 매출액
tg2 = sp[sp[:, 1] == 1, 0]  # 비올 때 매출액
print(tg1[:3])
print(tg2[:3])
...
# 두 집단의 각 평균
print('비 안올 때 : ', np.mean(tg1), ' ', '비 올 때 : ', np.mean(tg2))  #761040.2542372881   757331.5217391305
