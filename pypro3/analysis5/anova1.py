...
# 독립변수 : 범주형, 종속변수: 연속형
# f-value =  그룹 간 분산/ 그룹 내 분산
# 집단 간 분신이 집단 내 분산보다 충분히 큰 것인가를 파악하는 것.
...
# 귀무 : 세 가지교육방법에 따른 교육생 실기시험의 평균에 차이가 없다.
# 대립 : 세 가지교육방법에 따른 교육생 실기시험의 평균에 차이가 있다.
import pandas as pd
...
# 등분산 확인 : 만족하면 anova, 만족하지않으면 welch_anova
# 등분산을 만족하지 않으면 데이터 가공을 생각할 수도 있다. - 표준화, 정규화, transformation(자연 log를 씌움)
...
score2 = m2['score']
score3 = m3['score']
print('등분산성 확인: ', stats.levene(score1, score2, score3).pvalue)  # 0.11322850654055751 > 0.05 만족
...
# 사후검정(Post Hoc Test)
# 분산분석은 세 개 이상의 집단에 평균에 차이 유무만을 알려줌
