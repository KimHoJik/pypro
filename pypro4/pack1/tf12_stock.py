# 다중선형회귀 : 주식 데이터로 회귀모델 작성. 하루 전 데이터로 다음 날 종가 예측
...
print('r2_score : ', r2_score(y_data, pred))  # 0.9791 과적합이 의심스러움
...
print('split after - r2_score : ', r2_score(y_test, pred2))  # 0.7574 과적합 의심해제

print('------')
# fit(학습) 도중에 검증도 함께 하고자 할 경우, train data를 다시 분리해 vallidation data를 운영할 수있다.
# 과적합 방지 목적
model3 = Sequential()
model3.add(Dense(units = 1, input_dim = 4, activation='linear'))

model3.compile(optimizer='sgd',loss='mse', metrics=['mse'])
model3.fit(x_train, y_train, epochs=100, validation_split=0.3, verbose = 0)  # 학습은 train
print('evaluate : ', model2.evaluate(x_test, y_test))
