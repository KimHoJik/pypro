...
print('모델 작성방법 1 : Sequential api : 가장 일반적이고 단순한 방법')
model = Sequential()
...
print('실제 값 : ', y_data)
print('예측 값 : ', model.predict(x_data).flatten())
...
loss_metrics2 = model2.evaluate(x=x_data, y=y_data)
print('loss_metrics2 : ', loss_metrics2)

from sklearn.metrics import r2_score
print('설명력2 : ', r2_score(y_data, model2.predict(x_data)))
print('실제 값2 : ', y_data)
print('예측 값2 : ', model2.predict(x_data).flatten())

new_data = [1.5, 2.3, 5.8]
print('새 예측값2 : ', model2.predict(new_data).flatten())
...
loss_metrics3 = model3.evaluate(x=x_data, y=y_data)
...
loss_metrics4 = model4.evaluate(x=x_data, y=y_data)
