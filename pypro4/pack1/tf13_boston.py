...
print('설명력 : ', r2_score(y_test, model.predict(x_test)))  # 0.7376
"""
print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^;')
# concatenate : numpy 배열 결합
a = np.array([[1,2],[3,4]])  # 2by2 
b = np.array([[5,6],[7,8],[9,10]]) # 3by2
print(np.concatenate((a, b), axis = 0))  # 열기준
print(np.concatenate((a, b.T), axis = 1))  # 행기준
print(np.concatenate((a, b), axis = None))  # 첫번째 배열부터 가로방향 ->  차원 축소
print()
...
for i in range(k):
    # print('i:', i)
    # train data의 일부를 validation data로 사용하기 위해 데이터 추출  : 전체 데이터 수 - 404
    ...
    #print(val_x.shape, ' ', val_y.shape)  # (101, 13)   (101,)
    # print(val_x[:1])
...
print('kfold 전체 평균 : ', np.mean(all_mse_history))
from sklearn.metrics import r2_score
print('설명력 : ', r2_score(y_test, model2.predict(x_test)))  # 0.7376
